...
trait UserAccess
{
     ...
     * @return bool
     */
    public function allow($nameOrId, $all = true)
    {

        // check site / course - `role_user` table to `permissions`
        foreach ($this->roles as $role) {
            // See if role has all permissions
        ...
        // check events - `event_user` table to `permissions`
        foreach ($this->event_roles as $event_role) {


            // Validate against the Permission table
            foreach ($event_role->permissions as $event_perm) {

                // First check to see if it's an ID
                if (is_numeric($nameOrId)) {
                    if ($event_perm->id == $nameOrId) {
                        return true;
                    }
                }

                // Otherwise check by name
                if ($event_perm->name == $nameOrId) {
                    return true;
                }
            }
        }
     ...
     * @return bool
     */
    public function hasPermission($nameOrId, $all = true)
    {
        return $this->allow($nameOrId, $all);
    }
