...
class StoreResourceSubCategoryRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'description' => ['required', 'max:50', Rule::unique('resource_sub_category')->where(function($query) {
                $query->where('resource_category_id', $this->resource_category_id);
            })],

            'abbrv' => ['required', 'max:25', Rule::unique('resource_sub_category')->where(function($query) {
                $query->where('resource_category_id', $this->resource_category_id);
            })],
