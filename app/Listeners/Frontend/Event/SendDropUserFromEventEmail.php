...
class SendDropUserFromEventEmail
{
    /**
     ...
     * @return void
     */
    public function handle(DropUserFromEvent $userDropEvent) {
        ...
        $eventRoleAssignedToUser = EventUser::where('user_id', $userDropEvent->user->id)
            ->where('event_id', $userDropEvent->event->id)
            ->where('role_id', $userDropEvent->role->id)
            ->withTrashed() // Need this since we just soft deleted the event_user before we called our event
        ...
        $dashboardUrl = $siteUrl."/courseInstance/events/event-dashboard/";

        $eventEmails = EventEmails::where('event_emails.event_id', $userDropEvent->event->id)
                                ->where('event_emails.email_type_id', 5) //Remove from Event
                                ->get();          
        ...
        foreach ($eventEmails as $email) {
            ...
            $toArr[] = $userDropEvent->user->email;
            // There are no other to's in an "Add to event". The to field just has the user that was added.
         
            // Do same for CC'c
            $ccs = $this->roles->getRoleEmails($email->cc_roles, $email->course_id, $userDropEvent->event->id);
            $ccArr = $ccs;
            ...
            // Do same for BCC'c
            $bccs = $this->roles->getRoleEmails($email->bcc_roles, $email->course_id, $userDropEvent->event->id);
            $bccArr = $bccs;
            ...
            $subjectArr = [
                ...
                '</p>' => '',
                '&nbsp;' => ' ',
                '{{first_name}}'   => $userDropEvent->user->first_name,
                '{{last_name}}'    => $userDropEvent->user->last_name,
                ...
                '{{building_more_info}}'  => str_replace(array_keys($rPTag), array_values($rPTag), $email->event->InitialMeetingRoom->Location->Building->more_info),                '{{role}}' => $userDropEvent->role->name,     
            ];
            ...
            $sent = SentEmails::create([
                ...
                'primary_recipient'=> $userDropEvent->user->email,
                'to' => implode(', ', $toArr),
                'cc' => implode(', ', $ccArr),
                'bcc' => implode(', ', $bccArr),
