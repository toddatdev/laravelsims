...
namespace App\Listeners\Backend\Course;

use Illuminate\Queue\InteractsWithQueue;
use Illuminate\Contracts\Queue\ShouldQueue;
use App\Events\Backend\Course\SendManualEmail;
use App\Mail\SendCourseEmail; // This Class just creates a view to add our email to
...
class SendEmailManually
{
    ...
    public function handle(SendManualEmail $event) {
        ...
        //dd($subjectConversion);
        $sent = SentEmails::create([
            'site_email_id' => null,
            'course_email_id' => $event->email->course_email_id,
            'event_email_id' => null,
            'primary_recipient'=> 'course notification',
            'to' => implode(', ', $toArr),
            'cc' => implode(', ', $ccArr),
            'bcc' => implode(', ', $bccArr),
        ...
        \Mail::to($toArr)->cc($ccArr)->bcc($bccArr)->send(new SendCourseEmail($event->email));
