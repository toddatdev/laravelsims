            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">{{ trans('labels.backend.access.users.edit') }}</h3>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            {{ Form::label('first_name', trans('validation.attributes.backend.access.users.first_name'), ['class' => 'col-lg-2 control-label text-md-right']) }}

                            <div class="col-lg-10">
                                {{ Form::text('first_name', null, ['class' => 'form-control', 'maxlength' => '30', 'required' => 'required', 'autofocus' => 'autofocus', 'placeholder' => trans('validation.attributes.backend.access.users.first_name')]) }}
                            </div><!--col-lg-10-->
                        </div><!--form control-->

                        <div class="form-group row">
                            {{ Form::label('middle_name', trans('validation.attributes.backend.access.users.middle_name'), ['class' => 'col-lg-2 control-label text-md-right']) }}

                            <div class="col-lg-10">
                                {{ Form::text('middle_name', null, ['class' => 'form-control', 'maxlength' => '30', 'autofocus' => 'autofocus', 'placeholder' => trans('validation.attributes.backend.access.users.middle_name')]) }}
                            </div><!--col-lg-10-->
                        </div><!--form control-->
                        ...
                        <div class="form-group row">
                            {{ Form::label('email', trans('validation.attributes.backend.access.users.email'), ['class' => 'col-lg-2 control-label text-md-right']) }}
                        ...
                        @if ($user->id != 1)
                            ...
                            <div class="form-group row">
                                {{ Form::label('associated_roles', trans('validation.attributes.backend.access.users.associated_roles'), ['class' => 'col-lg-2 control-label text-md-right']) }}

                                <div class="col-lg-8">
                                    @if (count($roles) > 0)
                                        @foreach($roles as $role)

                                            {{--mitcks 9/23/19 disable the checkbox for the User role because it cannot be removed, but display it so there is always at least one selected--}}
                                            @if ($role->id == $siteUserRoleId)
                                                <input disabled type="checkbox" value="{{$role->id}}" name="assignees_roles[{{ $role->id }}]"
                                                       {{ is_array(old('assignees_roles')) ?
                                                       (in_array($role->id, old('assignees_roles')) ? 'checked' : '')
                                                       : ($user->roles->contains($role->id)? 'checked' : '') }}
                                                       id="role-{{$role->id}}" />
                                                {{--Hidden field here for User role only because disabled checkbox not included in post,
                                                if the user is not in the User role for some reason, it will also add them so they have at least 1 site role--}}
                                                {{ Form::hidden('assignees_roles[' . $role->id . ']', $role->id) }}
                                            @else
