  ...
  function createSlider(vals) {
    ...
    window.slider.on('update',function(values,handle){
      ...
      if(handle === 0) {
        ...
        //sets first row of resource template section when creating from scratch
        if (window.location.href.indexOf("create-from-scratch") > -1) {
            $("input[name='0_start_time']").val(moment(values[0], 'LT').format('h:mm A'));
        }
      ...
      if(handle === 1) {
        ...
        //sets first row of resource template section when creating from scratch
        if (window.location.href.indexOf("create-from-scratch") > -1) {
            $("input[name='0_end_time']").val(moment(values[1], 'LT').format('h:mm A'));
        }
      }
