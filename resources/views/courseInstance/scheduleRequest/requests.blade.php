                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="requests-table" class="table table-condensed table-hover dt-responsive nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        <th class="all">{{ trans('labels.scheduling.course') }}</th>
                                        <th></th> {{--hidden for icons--}}
                                        <th class="all">{{ trans('labels.scheduling.event_time')}}
                                        <th></th>
                                        <th></th>
                                        <th>{{ trans('labels.calendar.location') }}</th>
                                        <th>{{ trans('labels.event.event_requested_by') }}</th>
                                        ...
                                        <th class="all"></th> {{-- This is the notes field, which is hidden, but searchable. -jl --}}
                                    </tr>
                                </thead>
                            </table>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="panel panel-default" id="no-schedule-view">
                                <div class="panel-body">
                                    <h3 class="text-center">{{ trans('labels.scheduling.requests_please_select') }}</h3>
                                </div>
                            </div>
...
@section('after-scripts')
    ...
    <script>
        ...
        $(function() {
            ...
            let request_id = "{{ app('request')->input('id') }}";
            if (request_id) {
                $('#modal-link').attr("href", "request/"+request_id)
                $('#modal-link').click();
            }
            ...
            $('#requests-table tbody').on( 'click', 'tr', function (e) {
                actionSelected();
            });
        });
        ...
        // TC 10/03/2019 - Not sure why this is in a timeout, but cannot get row  data w/out
        function actionSelected() {
            setTimeout(function(){
                ...
                // OLD CODE
                // dp.startDate = dateFormatted;
                // getEventsAndResourcesByDate(dateFormatted);
            }, 2000);
        }


        // header buttons trigger their specific view functions first id is normal, second is mobile
        $('#pending, #pending-mobile').click(function() {
            pending();
        });

        $('#approved, #approved-mobile').click(function() {
            approved();
        });

        $('#denied, #denied-mobile').click(function() {
            denied();
        });

        $('#all, #all-mobile').click(function() {
            all();
        });
        ...
        $("body").on("click", "a[name='deny']", function(e) {
            e.preventDefault();
            ...
            //pop up
            swal({
                title: "{{ trans('labels.scheduling.requests_deny_wall') }}  " + abbrv + " on " + start_date + "?",
                text: message,
                icon: "warning",
                buttons: true,
                dangerMode: true,
        ...
        //pending requests
        function pending() {
            //change panel heading and hide the filter for pending view
            document.title = "{{ trans('labels.scheduling.pending_requests') }}"
            $('#heading').text("{{ trans('labels.scheduling.pending_requests') }}");
            $('#date-filter').hide();
        ...
        //approved requests
        function approved() {
            document.title = "{{ trans('labels.scheduling.approved_requests') }}"
            $('#heading').text("{{ trans('labels.scheduling.approved_requests') }}");
            $('#date-filter').show();
        ...
        // denied requests
        function denied() {
            document.title = "{{ trans('labels.scheduling.denied_requests') }}"
            $('#heading').text("{{ trans('labels.scheduling.denied_requests') }}");
            $('#date-filter').show();
        ...
        function init() {
            ...
            dt = $('#requests-table').DataTable({
                ...
                columns: [
                    ...
                    { data: 'sim.spec.needed', name:'sim.spec.needed', orderable: false, searchable: false },
                    ...
                    { data: 'notes', name: 'notes', visible: false, searchable: true },

                ],
                columnDefs: [
                    { targets: [8], className: 'dt-body-center'},
                    { targets: 2, orderData: [3,4]} //sort event date time by hidden columns start_time, end_time
                ],
            });
        ...
        $("#commentsModal").on('show.bs.modal', function (e) {
            ...
            $(this).find(".modal-body").load("request/"+id);
        });
        ...
        $("#denyModal").on('show.bs.modal', function (e) {
            ...
            $(this).find(".modal-body").load("denyModal/"+id);
        });
        ...
        var pageFilters = [];
        var courses = [];
        var endHours = {{ $businessBeginHour}};
        var endTime = "T"+ ((endHours+1) <=9 ? ('0'+(endHours+1)) : (endHours+1)) + ":00:00";
        var businessBeginHour = "T{{ $businessBeginHour }}:00:00";
        var businessEndHour = "T{{ $businessEndHour }}:00:00";
        var defaultStartTime = "{{ $defaultStartHour }}:00";
        var defaultEndTime = "{{ $defaultEndHour }}:00";
        var selectedStartTime = "T{{ $defaultStartHour }}:00:00"
        var selectedEndTime = "T{{ $defaultEndHour }}:00:00"
        var locationsAndResources = {!! $locationsAndResources !!}
        var existingEvents = [];
        var roDayPilot = true;

        initDayPilotDayView();
        getEventsAndResourcesByDate(selectedDate);
                ...
                if(thisEvent.event_resources.length) {
                    for(let j = 0; j < thisEvent.event_resources.length; ++j) {
                        let thisResource = thisEvent.event_resources[j];
                            let start = new DayPilot.Date(thisResource.start_time).addMinutes(-Math.abs(thisResource.setup_time));
                            let end = new DayPilot.Date(thisResource.end_time).addMinutes(Math.abs(thisResource.teardown_time));
